...
class WechatAuth {
    ...
    public function getAccessToken($type = 'client', $code = null){
        ...
        switch ($type) {
            case 'client':
                // 从缓存中读取access_token信息
                $token = $this->cache($type_id);
        ...
        if(is_array($token)){
            ...
            if(isset($token['errcode'])){
                ...
                if($type == 'client'){
                    $this->cache($type_id, $token['access_token']);
    ...
    private function cache($type, $val = null){
        $appid = $this->appId.'_'.$type;
        
        $Model = M('wx_token');
        $data = $Model->find($appid);
        ...
        if(is_null($val)){
            ...
            $save = array('appid' => $appid, 'val' => $val, 'exp' => time() + 7060);
            if(empty($data)){
                $Model->add($save);
            }else{
                $Model->save($save);
