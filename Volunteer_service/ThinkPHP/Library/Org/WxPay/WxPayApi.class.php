...
class WxPayApi
{
	...
	public static function unifiedOrder($inputObj, $timeOut = 6)
	{
		...
		if(!$inputObj->IsNotify_urlSet()){
			$inputObj->SetNotify_url(C('WEIXIN.notify_url'));//异步通知url
		}
		
		$inputObj->SetAppid(C('WEIXIN.appid'));//公众账号ID
		$inputObj->SetMch_id(C('WEIXIN.mch_id'));//商户号
	...
	{
		//如果不需要上报数据
		$error_levenl = C('WEIXIN.erport_levenl');
		if($error_levenl == 0){
			return;
		} 
		//如果仅失败上报
	...
	{		
		...
		//如果有配置代理这里就设置代理
		$curl_proxy_host = C('WEIXIN.curl_proxy_host');
		$curl_proxy_port = C('WEIXIN.curl_proxy_port');
		...
		if($useCert == true){
			...
			curl_setopt($ch,CURLOPT_SSLCERT, C('WEIXIN.cert').'/apiclient_cert.pem');
			curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
			curl_setopt($ch,CURLOPT_SSLKEY, C('WEIXIN.cert').'/apiclient_key.pem');
		}
		//post提交方式
