<?php if (!defined('THINK_PATH')) exit();?><form id="submit" method="post" action="/admin/teacheruser/importfile" enctype="multipart/form-data" data-validate="true" data-submit="ajax" class="form-horizontal edit-form modal modal-small hide fade" tabindex="-1" role="dialog" aria-hidden="true" data-continue="true" style="width:300px;margin-left: -150px;">

	<div class="widthexcel">
		<div class="titleExcel">导入老干部信息(excel格式)</div>
		<div class="clearfixed excelCon">
			<div class="li"><a href="/import_template/导入老干部信息表.xls" class="excelBtn">下载模板</a></div>
			<div class="li" style="margin-left: 20px;"><a href="javascript:void(0);" class="excelBtnFile" id="file" data-dismiss="modal" aria-hidden="true">导入老干部信息表</a></div>
		</div>
	</div>
</form>
	...
	$('#file').on('click', function(){
		...
		editor.addListener('afterUpfile', function (t,list) {
			...
			$('#edui7_state').click(function(){
				$.ajax({
			        url: '/admin/teacheruser/importfile',
			        type: 'post',
			        data: {
			        	url:url
			        },
